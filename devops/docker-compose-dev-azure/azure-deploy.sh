...
resourcename="dinos"  # lower case, only
resourcegroup="dinos_rg"
...
# git clone https://github.com/apilogicserver/dinos.git
# cd dinos
...
az group create --name dinos_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group dinos_rg --sku S1 --is-linux
az container create --resource-group dinos_rg --name dinos --image apilogicserver/dinos:latest --dns-name-label dinos --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//dinos.westus.azurecontainer.io:5656'
